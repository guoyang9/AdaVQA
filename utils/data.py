...
class VQA(data.Dataset):
	...
	def __init__(self, questions_path, answers_path,
		...
		# t
		self.margins = self._encode_margin(self.vocab['type'])

		# qt
		self.q_types = [ans['question_type'] for ans in answers_json]
	...
	def _encode_margin(self, type):
		""" Load answer margin per question type. """
		margins = {}
		for qt in type:
			ans_num_dict = utils.json_keys2int(type[qt])
	...
	def __getitem__(self, item):
		...
		m = torch.zeros(config.max_answers)
		m.scatter_(0, margin_label, margin_score)

		return item, v, q, a, b, m, q_len
...
def prepare_mul_answers(answers_json):
	...
	answers = [ans_dict['multiple_choice_answer'] for ans_dict in answers_json]
